	...
	<div class="flex-1 overflow-auto">
			...
			<div class="divide-y divide-border/50">
				...
				{#each [['Vcore', `${sensorData.vCore}V`, 0.9, 1.4], ['+3.3V', `${sensorData.v33}V`, 3.1, 3.5], ['+5V', `${sensorData.v5}V`, 4.7, 5.3], ['+12V', `${sensorData.v12}V`, 11.5, 12.5]] as [name, value, lo, hi] (name)}
					<div class="flex items-center gap-4 px-5 py-2">
						...
						<span class="w-28 shrink-0 text-xs text-muted-foreground">{name}</span>
						<span class="text-xs text-muted-foreground">{lo}V</span>
						<div class="h-1 flex-1 bg-muted">
							<div
						...
						<span class="w-16 shrink-0 text-right font-mono text-xs font-medium text-foreground"
							>{value}</span
						>
					</div>
				{/each}
				...
				<div class="flex items-center justify-between px-5 py-2.5">
					<span class="text-xs text-muted-foreground">Current Draw</span>
					<span class="text-xs font-medium text-foreground">{colo.selectedUnit.powerDraw}</span>
				</div>
				<div class="flex items-center justify-between px-5 py-2.5">
					<span class="text-xs text-muted-foreground">Budget</span>
					<span class="text-xs font-medium text-foreground">{colo.selectedUnit.powerBudget}</span>
				</div>
				<div class="px-5 py-3">
		...
		{:else}
			<div class="flex flex-col items-center justify-center py-20 text-muted-foreground">
				...
				<p class="mt-1 text-xs text-muted-foreground">Power on via IPMI to view sensor data.</p>
			</div>
		{/if}
	</div>
{/if}
