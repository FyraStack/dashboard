...
<div class="flex flex-1 overflow-hidden">
	...
	<div class="flex w-64 shrink-0 flex-col border-r border-fyra-gray-800">
		...
		<div class="flex-1 overflow-y-auto">
			{#each groups as group (group.id)}
				<button
					class="flex w-full items-center justify-between border-b border-fyra-gray-800 px-4 py-3 text-left transition-colors duration-100 {selectedGroupId ===
					group.id
						? 'bg-fyra-gray-800/60'
				...
				>
					<div class="min-w-0">
						<p class="truncate text-sm font-semibold text-fyra-gray-100">{group.name}</p>
						<p class="mt-0.5 text-xs text-fyra-gray-500">
							{group.rules.length} rules &bull; {group.servers.length} servers
						</p>
					</div>
					<ChevronRight class="h-3.5 w-3.5 shrink-0 text-fyra-gray-600" />
				</button>
			{/each}
	...
	{#if selectedGroup}
		<div class="flex flex-1 flex-col overflow-hidden">
			...
			>
				<div class="flex items-center gap-2">
					<span class="text-sm font-medium text-fyra-gray-200">{selectedGroup.name}</span>
					<span class="text-fyra-gray-600">&middot;</span>
					<div class="flex gap-1">
						{#each selectedGroup.servers as s (s)}
							<Badge variant="secondary" class="text-[10px]">{s}</Badge>
						{/each}
						{#if selectedGroup.servers.length === 0}
							<span class="text-xs text-fyra-gray-500">No servers assigned</span>
						{/if}
					</div>
				</div>
					...
					<tbody class="divide-y divide-fyra-gray-800/30">
						{#each selectedGroup.rules.filter((r) => r.direction === 'inbound') as rule (rule.id)}
							<tr class="transition-colors duration-100 hover:bg-fyra-gray-800/20">
								...
								<td class="px-5 py-2.5 font-mono text-sm text-fyra-gray-200">{rule.ports}</td>
								<td class="px-5 py-2.5 font-mono text-sm text-fyra-gray-400">{rule.source}</td>
								<td class="px-5 py-2.5">
						...
						{#if selectedGroup.rules.filter((r) => r.direction === 'inbound').length === 0}
							<tr>
								<td colspan="5" class="px-5 py-6 text-center text-xs text-fyra-gray-500"
									>No inbound rules</td
								>
							</tr>
						{/if}
	...
	<Dialog.Content class="border-fyra-gray-800 bg-fyra-gray-900 sm:max-w-md">
		...
		<div class="flex flex-col gap-4 py-4">
			...
			<div class="flex flex-col gap-2">
				...
				<p class="text-xs text-fyra-gray-500">Single port, comma-separated, or range.</p>
			</div>
			<div class="flex flex-col gap-2">
...
<Dialog.Root bind:open={createGroupOpen}>
	<Dialog.Content class="border-fyra-gray-800 bg-fyra-gray-900 sm:max-w-md">
		...
		<div class="flex flex-col gap-4 py-4">
			<div class="flex flex-col gap-2">
				<Label>Name</Label>
				<Input bind:value={newGroupName} placeholder="my-firewall" />
			</div>
		</div>
		<Dialog.Footer>
			<Button variant="outline" size="sm" onclick={() => (createGroupOpen = false)}>Cancel</Button>
